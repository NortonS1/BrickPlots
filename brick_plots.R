...
image_location <- paste("images", format(Sys.time(), "%d-%b-%Y %H.%M.%S"), sep = "_")
dir.create(image_location)
...
jpeg(filename = paste("elbow_point", ".jpeg"), width = 500, height = 500)
...
jpeg(filename = paste("initial_locations", ".jpeg"), width = 500, height = 500)
...
for (cluster in 1:length(scaled_data[, 1])) {
  print(paste("plotting cluster", all_data[cluster,1], sep = " "))
  #initiating image file
  jpeg(filename = paste(all_data[cluster,1], ".jpeg"), width = 1000, height = 1000)
